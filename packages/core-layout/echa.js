...
UI.Eacha = function (argFunc, contentBlock, elseContentBlock) {
  checkBlockHelperArguments('Eacha', argFunc, contentBlock, elseContentBlock);
  return UI.EachaImpl.extend({
    __sequence: argFunc,
...
UI.EachaImpl = UI.Component.extend({
  typeName: 'Eacha',
  ...
  parented: function () {
    ...
    this.observeHandle = ObserveSequence.observe(function () {
      ...
      addedAt: function (id, item, i, beforeId) {
        ...
        var renderedItem = UI.render(content.extend({data: dataFunc, __aid: i}), self);
